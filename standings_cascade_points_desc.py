    ...
    if os.path.exists(ADJUSTMENTS_FILE):
        try:
            with open(ADJUSTMENTS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                return (
                    data.get("TEAM_RECORD_ADJUSTMENTS", {}),
                    data.get("TEAM_POINT_ADJUSTMENTS", {})
                )
        except Exception as e:
            print(f"[ERROR] No se pudo leer {ADJUSTMENTS_FILE}: {e}")
            return {}, {}
    else:
        print(f"[WARN] {ADJUSTMENTS_FILE} no encontrado.")
        return {}, {}

TEAM_RECORD_ADJUSTMENTS, TEAM_POINT_ADJUSTMENTS = load_adjustments()
